...
def get_messages(session_id: str) -> list[dict[str, Any]]:
    ...
    result = []
    for row in rows:
        payload = {"id": row.id, "session_id": row.session_id, "role": row.role, "content": row.content, "timestamp": row.timestamp}
        if row.tool_call_id: payload["tool_call_id"] = row.tool_call_id
        if row.tool_calls:
            try: payload["tool_calls"] = json.loads(row.tool_calls)
            except: payload["tool_calls"] = row.tool_calls
...
def _build_agent_components(callbacks: AgentCallbacks) -> tuple[AgentLoop, DualTrackRAG]:
    ...
    rag = DualTrackRAG(docs_path=DOCS_PATH, translate_api_key=config.dashscope_api_key, translate_base_url=config.dashscope_base_url, translate_model=config.dashscope_translate_model, selection_context_provider=lambda: registry.execute_tool("query_selection_context", {}))
    loop = AgentLoop(llm_client=llm_client, tool_executor=registry.execute_tool, callbacks=callbacks, system_prompt=SYSTEM_PROMPT, tool_schemas=registry.get_all_schemas(), max_history_messages=config.agent_max_history_messages, tool_repeat_limit=config.agent_tool_repeat_limit, is_tool_dangerous=registry.is_dangerous_tool)
    return loop, rag
...
def _run_turn_sync(session_id: str, user_text: str, use_rag: bool, callbacks: WebSocketAgentCallbacks) -> None:
    try:
        ...
        # 【修复 1】在此处提前发状态，缓解半分钟白屏焦虑
        if use_rag:
            callbacks._emit({"type": "status_update", "content": "✨ 正在通过 RAG 解析场景上下文与意图..."})
        injected_context = rag.build_injected_context(user_text) if use_rag else ""
        
        callbacks._emit({"type": "status_update", "content": "🧠 正在思考与制定执行计划..."})
        ...
        for msg in new_messages:
            ...
            if role == "user" and not skipped_user:
                skipped_user = True
                continue
            db.append_message(session_id=session_id, role=role, content=msg.get("content", ""), tool_calls=msg.get("tool_calls"), tool_call_id=msg.get("tool_call_id"))
